...
from data.dssp_constants import (
    NUM_DSSP_CLASSES, DSSP_TO_IDX, FIXED_DSSP_CLASS_WEIGHTS
)
class DFMNodeClassifier(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_dssp_loss:
            ...
            if FIXED_DSSP_CLASS_WEIGHTS is not None:
                self.register_buffer('dssp_class_weights', FIXED_DSSP_CLASS_WEIGHTS.clone())
            else:
        ...
        if self.use_dssp_loss:
            ...
            self.shared_head, self.skip_weights = self._build_prediction_head(
                combined_hidden_dim, head_hidden, head_dropout, head_depth, head_hidden  # shared output = head_hidden
            )
            ...
            self.head, self.skip_weights = self._build_prediction_head(
                combined_hidden_dim, head_hidden, head_dropout, head_depth, sequence_output_dim
            )
    ...
    def _build_prediction_head(self, input_dim: int, hidden_dim: int, dropout: float, depth: int, output_dim: int):
        """
        ...
        if depth == 1:
            ...
            for group_idx in range(0, depth - 1, 3):  # Every 3 groups, excluding final layer
                if group_idx == 0:
    ...
    def _apply_head_with_time_conditioning(self, hs, node_time_emb, t_emb, batch_mask, return_both_heads=False):
        """
        ...
        if self.use_time_conditioning:
            if self.head_depth == 1:
                ...
                if layer_idx < len(head_layers) and isinstance(head_layers[layer_idx], nn.Dropout):
                ...
                while layer_idx < len(head_layers) - 1:  # Exclude final layer
                    ...
                    if isinstance(head_layers[layer_idx], nn.Linear):
                        ...
                        # Apply activation
                        if layer_idx < len(head_layers) and isinstance(head_layers[layer_idx], nn.LeakyReLU):
                            h = head_layers[layer_idx](h)
                            layer_idx += 1
                            
                            # Check if we should collect skip feature after activation
                            if hasattr(self, 'skip_points') and (layer_idx - 1) in self.skip_points:
                                skip_features.append(h.clone())
                ...
                if hasattr(self, 'has_skip_connections') and self.has_skip_connections and len(skip_features) > 0:
                    ...
                    weighted_features = torch.stack([w * feat for w, feat in zip(weights, all_features)], dim=0)
            ...
            if self.head_depth == 1:
                ...
                while layer_idx < len(head_layers) - 1:
                    ...
                    if (hasattr(self, 'skip_points') and layer_idx in self.skip_points and 
                        isinstance(head_layers[layer_idx], nn.LeakyReLU)):
                        skip_features.append(h.clone())
    ...
    def forward(self, data, t, prob_t, return_dssp=None):
        ...
        if hasattr(data, 'batch') and data.batch is not None:
            ...
            batch_mask = torch.zeros(total_nodes, dtype=torch.long, device=data.x_s.device)
        ...
        for i in range(B):
            ...
            if num_nodes_in_graph > 0:
                ...
                seq_probs = prob_t[i]  # [N, K] - these are probabilities from Dirichlet sampling
                ...
                if actual_len > 0:
                    ...
                    assert n_real >= actual_len, \
                        f"Expected at least {n_real} real nodes, but got {actual_len}"
                    if n_real > actual_len:
                        uniform_probs = torch.full((n_real - actual_len, seq_probs.size(-1)), 
                                                 1.0 / seq_probs.size(-1),
                                                 device=seq_probs.device, dtype=seq_probs.dtype)
                        real_probs = torch.cat([real_probs, uniform_probs], dim=0)
                    
                    # Virtual node probabilities (uniform distribution)
                    virtual_probs = torch.full((1, seq_probs.size(-1)), 
                                             1.0 / seq_probs.size(-1),
                                             device=seq_probs.device, dtype=seq_probs.dtype)
                    
                    # Combine real and virtual probabilities
                    graph_probs = torch.cat([real_probs, virtual_probs], dim=0)  # [num_nodes_in_graph, K]
                else:
                    # All nodes get uniform probabilities
                    graph_probs = torch.full((num_nodes_in_graph, seq_probs.size(-1)), 
                                           1.0 / seq_probs.size(-1),
                                           device=seq_probs.device, dtype=seq_probs.dtype)
        ...
        if node_prediction_probs:
            initial_prediction_probs = torch.cat(node_prediction_probs, dim=0)  # [total_nodes, K]
        else:
            raise Exception("Input probabilities should exist.")
            # Fallback: uniform probabilities for all nodes
            initial_prediction_probs = torch.full((data.x_s.size(0), 21), 
                                                1.0 / 21.0,
                                                device=data.x_s.device, dtype=data.x_s.dtype)
        ...
        for recycle_iter in range(self.recycle_steps):
            ...
            hs = self.gnn(data, predictions=current_prediction_probs, 
                         time_emb=node_time_emb)
            ...
            if recycle_iter < self.recycle_steps - 1:
                ...
                current_prediction_logits = self._apply_head_with_time_conditioning(hs, node_time_emb, t_emb, batch_mask)
                current_prediction_probs = torch.softmax(current_prediction_logits, dim=-1)

        # Step 3: Final prediction with time conditioning
        # Determine if we should return DSSP predictions
        should_return_dssp = return_dssp if return_dssp is not None else self.use_dssp_loss
        ...
        if should_return_dssp and self.use_dssp_loss:
            ...
            if sequence_logits.dim() != 2 or sequence_logits.size(-1) != 21:
                raise ValueError(f"Sequence output has wrong shape: {sequence_logits.shape}, expected [total_nodes, 21]")
            if dssp_logits.dim() != 2 or dssp_logits.size(-1) != NUM_DSSP_CLASSES:
                raise ValueError(f"DSSP output has wrong shape: {dssp_logits.shape}, expected [total_nodes, {NUM_DSSP_CLASSES}]")
            
            return sequence_logits, dssp_logits
        else:
            # Get only sequence predictions
            sequence_logits = self._apply_head_with_time_conditioning(hs, node_time_emb, t_emb, batch_mask)
            
            # Ensure correct shape
            if sequence_logits.dim() != 2 or sequence_logits.size(-1) != 21:
                raise ValueError(f"Model output has wrong shape: {sequence_logits.shape}, expected [total_nodes, 21]")
    ...
    def compute_loss(self, data, prob_t, y, t, dssp_targets=None, use_flexible_loss_scaling=False, precomputed_logits=None):
        """
        ...
        if precomputed_logits is not None:
            ...
            forward_output = self.forward(data, t, prob_t, return_dssp=self.use_dssp_loss)
        ...
        if use_flexible_loss_scaling:
            ...
            sequence_loss = self._weighted_cross_entropy_loss(sequence_logits, y, uncertainty_weights)
        else:
