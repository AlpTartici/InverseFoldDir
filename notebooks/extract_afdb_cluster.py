...
print(f"Loaded {len(dict_pdb_uniprot)} UniProt entries from the dictionary. Proceeding with AFDB50 cluster extraction...")
df_af_clusters_mem = pd.read_csv('../datasets/af_clusters/7-AFDB50-repId_memId.tsv.gz', sep='\t', compression='gzip', header=0)

df_af_clusters_mem.columns = ['repID', 'memID', 'idk']

print("AFDB50 cluster data loaded. Extracting representative IDs for each UniProt entry...")
...
for pdb_id in tqdm(dict_pdb_uniprot.keys()):
    ...
    if not repID.empty:
        dict_repID[pdb_id] = {'cluster_repID': repID.values[0], 'uniprot_id': uniprot_id}
    else:
        dict_repID[pdb_id] = None
# save the list to a file
with open('../datasets/af_clusters/afdb_clusters_repid_for_test_and_val.json', 'w') as f:
