...
class PipelineOrchestrator:
    ...
    def setup_output_directory(self):
        ...
        try:
            ...
            logger.warning(
                f"Cannot create output directory {self.config.base_output_dir} - assuming it will be created by AMLT"
            )
        ...
        try:
            config_path = os.path.join(
                self.config.base_output_dir, self.config.pipeline_config_save_filename
            )
            with open(config_path, "w") as f:
                json.dump(asdict(self.config), f, indent=2)
        except (PermissionError, FileNotFoundError):
            logger.warning(
                f"Cannot write configuration file to {self.config.base_output_dir} - continuing without saving config"
            )
    ...
    def resolve_stage_output_directories(self):
        ...
        # Debug logging
        logger.info(
            f"Resolving stage directories. base_output_dir: {self.config.base_output_dir}"
        )
        logger.info(
            f"Training output_dir before resolution: {self.config.training.output_dir}"
        )
        ...
        if not os.path.isabs(self.config.training.output_dir):
            ...
            logger.info(
                f"Training output_dir after resolution: {self.config.training.output_dir}"
            )
    ...
    def setup_logging(self):
        ...
        try:
            ...
            logger.warning(
                f"Cannot write log file to {log_file} - logging only to console"
            )
    ...
    def copy_key_results_to_output_dir(self, stage_name: str, stage_output_dir: str):
        ...
        if not os.path.exists(stage_output_dir):
            logger.warning(
                f"Stage output directory {stage_output_dir} does not exist, skipping copy"
            )
            return
        ...
        for file_pattern in files_to_copy:
            ...
            if "*" in file_pattern:
                ...
                for src_path in matching_files:
                    ...
                    try:
                        ...
                        dest_dir = os.path.join(
                            self.config.base_output_dir, f"{stage_name}_results"
                        )
                ...
                if os.path.exists(src_path):
                    ...
                    logger.debug(
                        f"Key file {file_pattern} not found in {stage_output_dir}"
                    )

        if copied_files:
            logger.info(
                f"Copied {len(copied_files)} key result files from {stage_name} to main output directory"
            )
        else:
    ...
    def run_command(
        ...
        try:
            ...
            logger.warning(
                f"Cannot create log directory {stage_log_dir} - logs will not be saved"
            )
            write_logs = False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"[{stage_name}] Command failed with return code {result.returncode}"
                )
                ...
                if capture_output and result.stderr:
                    ...
                    with open(stderr_file, "r") as f:
                        ...
                        if error_content:
                            logger.error(
                                f"[{stage_name}] Error output: {error_content}"
                            )
    ...
    def find_latest_checkpoint(self, checkpoint_dir: str) -> Optional[str]:
        ...
        if not checkpoint_files:
            ...
            print(
                f"[DEBUG] No .pt/.pth/.ckpt files found in {checkpoint_dir}", flush=True
            )
            return None
        ...
        if tmp_path:
            print(
                f"[DEBUG] Successfully copied checkpoint to tmp: {tmp_path}", flush=True
            )
    ...
    def _get_process_seed(self) -> int:
        ...
        base_seed = int(time.time() * 1000) % (
            2**31 - 1
        )  # Use milliseconds, keep within int32 range
        process_seed = (base_seed + os.getpid()) % (
            2**31 - 1
        )  # Add process ID for uniqueness

        logger.info(
            f"Generated process seed: {process_seed} (base: {base_seed}, pid: {os.getpid()})"
        )
        return process_seed
    ...
    def validate_af2_configuration(self, config: TrainingConfig) -> bool:
        ...
        if af2_chunk_dir.startswith("$$") or af2_chunk_dir.startswith("/mnt/"):
            logger.info(
                f"AF2 chunk directory configured (remote path): {af2_chunk_dir}"
            )
        else:
    ...
    def run_training_stage(self) -> bool:
        ...
        critical_args = [
            ...
            "--output_dir",
            self.config.base_output_dir,  # Pass base output dir for model copying
        ]
        ...
        tmp_dir = os.path.abspath(
            config.checkpoint_copy_dir or self.config.tmp_checkpoint_dir
        )
        ...
        if (
            hasattr(config, "use_time_conditioning")
            and not config.use_time_conditioning
        ):
        ...
        logger.info(
            f"Training command will use output_dir: {self.config.base_output_dir}"
        )
        ...
        command.extend(
            [
                "--split_json",
                config.dataset.split_json,
                "--map_pkl",
                config.dataset.map_pkl,
            ]
        )
        ...
        if config.wandb_enabled:
            command.extend(["--wandb_project", config.wandb_project, "--use_wandb"])
        ...
        try:
            ...
            result = self.run_command(
                command, "training", timeout=config.distributed.timeout_minutes * 60
            )
            ...
            if result.returncode == 0:
                ...
                if not latest_checkpoint:
                    ...
                    if os.path.exists(tmp_dir):
                        logger.info(
                            "No checkpoint in primary location, checking tmp directory..."
                        )
    ...
    def run_sampling_stage(self) -> bool:
        ...
        if model_checkpoint == "auto":
            if (
                "training" in self.stage_results
                and self.stage_results["training"]["status"] == "success"
            ):
                ...
                if os.path.exists(tmp_dir):
                    ...
                    if os.path.exists(tmp_checkpoint):
                        logger.info(
                            f"Using tmp checkpoint for sampling: {tmp_checkpoint}"
                        )
                        model_checkpoint = tmp_checkpoint
                    else:
                        logger.info(
                            f"Tmp checkpoint not found, using primary: {model_checkpoint}"
                        )

            else:
                logger.error(
                    "Auto model checkpoint requested but training did not complete successfully"
                )
                return False
        ...
        if config.distributed.enabled:
            try:
                ...
                if not torch.cuda.is_available():
                    logger.error(
                        "CUDA not available but distributed GPU training requested"
                    )
                    return False
                gpu_count = torch.cuda.device_count()
                if gpu_count < config.distributed.num_gpus:
                    logger.error(
                        f"Requested {config.distributed.num_gpus} GPUs but only {gpu_count} available"
                    )
                    return False
                logger.info(
                    f"GPU validation passed: {gpu_count} GPUs available, using {config.distributed.num_gpus}"
                )
            except ImportError:
        ...
        if (
            hasattr(config, "integration_method")
            and config.integration_method is not None
        ):
        ...
        if (
            hasattr(config, "save_probabilities")
            and config.save_probabilities is not None
        ):
        ...
        while i < len(config.additional_args):
            ...
            if arg in structured_args_to_filter:
                ...
                if i + 1 < len(config.additional_args) and not config.additional_args[
                    i + 1
                ].startswith("--"):
        ...
        try:
            # Run sampling
            result = self.run_command(
                command, "sampling", timeout=config.distributed.timeout_minutes * 60
            )
            ...
            if result.returncode == 0:
                ...
                output_patterns = (
                    config.output_file_patterns
                    if hasattr(config, "output_file_patterns")
                    else (
                        ["*.csv", "*.json"]
                        if config.output_format == "both"
                        else [f"*.{config.output_format}"]
                    )
                )
                latest_output = None

                for pattern in output_patterns:
                    latest_output = self.find_latest_output_file(
                        config.output_dir, pattern
                    )
                    if latest_output:
                        break
    ...
    def run_evaluation_stage(self) -> bool:
        ...
        if config.compare_structures and config.reference_structures_dir:
            if not os.path.exists(config.reference_structures_dir):
                logger.error(
                    f"Reference structures directory not found: {config.reference_structures_dir}"
                )
                return False
            if not os.path.isdir(config.reference_structures_dir):
                logger.error(
                    f"Reference structures path is not a directory: {config.reference_structures_dir}"
                )
                return False
            # Check if directory contains any PDB files
            pdb_files = [
                f
                for f in os.listdir(config.reference_structures_dir)
                if f.endswith(".pdb")
            ]
            if not pdb_files:
                logger.error(
                    f"No PDB files found in reference structures directory: {config.reference_structures_dir}"
                )
                return False
            logger.info(
                f"Reference structures validated: {len(pdb_files)} PDB files found"
            )
        ...
        if predictions_csv == "auto":
            if (
                "sampling" in self.stage_results
                and self.stage_results["sampling"]["status"] == "success"
            ):
                ...
                logger.error(
                    "Auto predictions CSV requested but sampling did not complete successfully"
                )
                return False
        ...
        command.extend(
            [
                ...
                predictions_csv,
                "--reference_dir",
                config.reference_structures_dir,
                "--output-dir",
                config.output_dir,
                "--batch-size",
                str(config.batch_size),
        ...
        try:
            # Run evaluation
            result = self.run_command(
                command, "evaluation", timeout=config.distributed.timeout_minutes * 60
            )
            ...
            if result.returncode == 0:
                ...
                results_file = self.find_latest_output_file(
                    config.output_dir, config.comparison_results_filename
                )
                if not results_file:
                    results_file = self.find_latest_output_file(
                        config.output_dir,
                        (
                            config.output_file_patterns[0]
                            if config.output_file_patterns
                            else "*.csv"
                        ),
                    )
    ...
    def generate_final_report(self):
        ...
        report = {
            ...
            "elapsed_time_human": str(
                datetime.fromtimestamp(elapsed_time) - datetime.fromtimestamp(0)
            ).split(".")[0],
        ...
        report_path = os.path.join(
            self.config.base_output_dir, self.config.pipeline_report_filename
        )
        try:
            ...
            logger.warning(
                f"Cannot write report file to {report_path} - report not saved"
            )
...
Examples:
  ...
  python config_loader.py
    ...
    parser.add_argument(
        "--config-file", help="Path to configuration file (JSON, YAML, or TOML)"
    )
    ...
    parser.add_argument(
        "--af2_chunk_dir",
        help="Path to AF2 chunk directory for AF2 training (e.g., $$AMLT_DATA_DIR/datasets/af2_chunks/)",
    )
    parser.add_argument(
        "--af2_chunk_limit", type=int, help="Maximum number of AF2 chunks to load"
    )
    # NOTE: AF2 data is ALWAYS loaded lazily - no upfront loading option
    parser.add_argument(
        "--heterogeneous_batches",
        type=lambda x: x.lower() == "true",
        help="Create heterogeneous batches mixing AF2 and PDB",
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument(
        "--no_source_indicator",
        action="store_true",
        help="Disable data source indicators - use [1.0, 1.0] for all data sources",
    )
    parser.add_argument(
        "--max_edge_dist",
        type=float,
        help="Maximum distance cutoff (Angstroms) for edge creation. Overrides k_neighbors, k_farthest, k_random. Max 80 neighbors per node.",
    )

    # Stage control
    parser.add_argument(
        "--disable-training", action="store_true", help="Disable training stage"
    )
    parser.add_argument(
        "--disable-sampling", action="store_true", help="Disable sampling stage"
    )
    parser.add_argument(
        "--disable-evaluation", action="store_true", help="Disable evaluation stage"
    )

    # Override parameters
    parser.add_argument("--training-epochs", type=int, help="Override training epochs")
    parser.add_argument(
        "--sampling-num-samples", type=int, help="Override number of samples"
    )
    parser.add_argument(
        "--global-num-gpus", type=int, help="Override number of GPUs for all stages"
    )
    parser.add_argument(
        "--external-model-checkpoint",
        help="Path to external model checkpoint for sampling (supports AMLT $$ variables)",
    )
    parser.add_argument(
        "--protein-subset",
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="Timeout per protein in minutes for evaluation (default: 3.0)",
    )
    ...
    if args.config_file:
        ...
        if args.config not in CONFIGS:
            ...
            try:
                ...
                logger.error(
                    f"CRITICAL: Configuration {args.config} not found in CONFIGS"
                )
    ...
    try:
        ...
        if not isinstance(config.training.additional_args, list):
            raise TypeError(
                f"training.additional_args must be list, got {type(config.training.additional_args)}"
            )
        ...
        logger.error(
            f"Config attributes: {dir(config) if hasattr(config, '__dict__') else 'No attributes'}"
        )
    ...
    if args.max_edge_dist is not None:
        ...
        logger.info(
            f"Using distance-based edge building: max_edge_dist={args.max_edge_dist}Å"
        )
    ...
    if args.external_model_checkpoint:
        ...
        logger.info(
            f"Using external model checkpoint: {args.external_model_checkpoint}"
        )
    ...
    if args.af2_chunk_dir:
        ...
        if not hasattr(config, "training"):
            raise AttributeError(
                "Configuration missing 'training' attribute. Check config loading."
            )
        if not hasattr(config.training, "additional_args"):
            raise AttributeError(
                "Configuration training missing 'additional_args' attribute. Check config structure."
            )
        if not isinstance(config.training.additional_args, list):
            raise TypeError(
                f"training.additional_args must be a list, got {type(config.training.additional_args)}"
            )
        ...
        for i, arg in enumerate(config.training.additional_args):
            ...
            if arg == "--af2_chunk_dir":
                ...
                logger.info(
                    f"Replaced existing --af2_chunk_dir with {args.af2_chunk_dir}"
                )
            else:
        ...
        logger.info(
            "No AF2 chunk directory provided, checking if AF2 mode requires it..."
        )
        ...
        if not hasattr(config.training, "additional_args"):
            raise AttributeError(
                "CRITICAL: Configuration training missing 'additional_args' attribute"
            )
        if not isinstance(config.training.additional_args, list):
            raise TypeError(
                f"CRITICAL: training.additional_args must be a list, got {type(config.training.additional_args)}"
            )
        ...
        try:
            ...
            logger.error(
                f"This usually indicates a programming error in AF2 detection logic"
            )
        ...
        if af2_enabled:
            logger.error(
                "CRITICAL ERROR: AF2 mode enabled but --af2_chunk_dir not provided."
            )
            logger.error("AF2 training requires access to AF2 chunk files.")
            logger.error(
                "Please specify --af2_chunk_dir path for AF2 files (e.g., --af2_chunk_dir $$AMLT_DATA_DIR/datasets/af2_chunks/)"
            )
    ...
    config.base_output_dir = os.path.join(
        config.base_output_dir, f"{config.name}_{timestamp}"
    )
    ...
    try:
        ...
        if not isinstance(config.base_output_dir, str):
            raise TypeError(
                f"base_output_dir must be string, got {type(config.base_output_dir)}"
            )

        # Validate that we're not accidentally creating multiple orchestrator instances
        # (this was the source of the previous bug)
        logger.info(
            f"Creating single PipelineOrchestrator instance for config: {config.name}"
        )
        ...
        if not hasattr(orchestrator, "config"):
            raise RuntimeError(
                "PipelineOrchestrator creation failed - missing config attribute"
            )
        if not hasattr(orchestrator, "is_af2_mode_enabled"):
            raise RuntimeError(
                "PipelineOrchestrator creation failed - missing AF2 detection method"
            )
        ...
        logger.error(
            "This usually indicates missing required attributes in config or code"
        )
