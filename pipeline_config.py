...
class DatasetConfig:
    ...
    map_pkl: str = "datasets/cath-4.2/chain_set_map_with_b_factors_dssp.pkl"  # DSSP-enabled dataset
...
class TrainingConfig:
    ...
    dataset_path: str = "data/cath_dataset.py"  # Keep for reference but not passed to training script
    config_path: Optional[str] = None  # This will be passed as --config_file only if specified
    ...
    num_layers_gvp: Optional[int] = None  # Must be specified in additional_args (renamed from num_layers)
    num_layers_prediction: Optional[int] = None  # Must be specified in additional_args (new parameter)
    ...
    enable_checkpoint_rollback: Optional[bool] = None  # Must be specified in additional_args
    ...
    lambda_dssp_loss: Optional[float] = None  # Must be specified in additional_args (0.0 = disabled, 0.1 = 10% DSSP weight)
    ...
    def validate_required_fields(self):
        ...
        required_fields = ['batch_size', 'epochs', 'learning_rate', 'grad_clip', 
                          'time_integration', 'use_time_conditioning', 'use_virtual_node']
        missing_fields = []
        ...
        if missing_fields:
            raise ValueError(f"Required training fields not specified: {missing_fields}. "
        ...
        recommended_args = ['--num_workers', '--dropout', '--weight_decay', '--patience', 
                          '--scheduler_patience', '--num_layers_gvp', '--num_layers_prediction', '--run_mode']
        missing_recommended = []
        ...
        if missing_recommended:
            print(f"Warning: Recommended args not found in additional_args: {missing_recommended}")
...
class SamplingConfig:
    ...
    max_edge_dist: Optional[float] = None  # From model metadata only - distance cutoff in Angstroms
    ...
    def validate_required_fields(self):
        """Validate that all required fields are specified."""
        required_fields = ['num_samples', 'temperature', 'steps', 'T', 't_min', 'split',
                          'integration_method', 'save_probabilities', 'detailed_json',
                          'batch_size', 'max_workers', 'output_format', 'min_length', 'max_length']
        ...
        if missing_fields:
            raise ValueError(f"Required sampling fields not specified: {missing_fields}. "
    ...
    def validate_model_dependent_params(self, model_metadata: dict):
        ...
        required_model_params = ['use_virtual_node', 'k_neighbors', 'k_farthest', 'k_random',
                               'max_edge_dist', 'num_rbf_3d', 'num_rbf_seq']
        missing_params = []
        ...
        if missing_params:
            raise ValueError(f"Required model parameters not found in model metadata: {missing_params}. "
...
class EvaluationConfig:
    ...
    protein_subset_path: Optional[str] = None  # Path to .npy file with specific proteins to evaluate
    ...
    def validate_required_fields(self):
        """Validate that all required fields are specified."""
        required_fields = ['reference_structures_dir', 'predict_structures', 'compare_structures',
                          'batch_size', 'max_workers', 'overwrite_predictions']
        missing_fields = []
        ...
        if missing_fields:
            raise ValueError(f"Required evaluation fields not specified: {missing_fields}. "
...
class PipelineConfig:
    ...
    description: str = "Complete distributed training, sampling, and evaluation pipeline"
    ...
    _skip_validation: bool = False  # Internal flag to skip validation for default configs
    ...
    def __post_init__(self):
        ...
        self.training.checkpoint_dir = os.path.join(self.training.output_dir, "checkpoints")
        ...
        if (hasattr(self, 'strict_validation') and self.strict_validation and 
            not hasattr(self, '_skip_validation')):
...
SCALING_EXPERIMENT_CONFIG = PipelineConfig(
    ...
    evaluation=EvaluationConfig(enabled=False)  # Disable evaluation for scaling experiments
)
