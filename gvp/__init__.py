...
# torch-scatter is a compiled extension without prebuilt wheels for every
# platform, and it is the most common installation failure on macOS and
# Windows. It is used here for a single scatter-add, which plain PyTorch can
# do natively, so fall back rather than making it a hard requirement.
try:
    from torch_scatter import scatter_add
except ImportError:  # pragma: no cover - exercised only without the extension
    def scatter_add(src, index, dim=0, dim_size=None):
        """Minimal stand-in for torch_scatter.scatter_add."""
        if dim_size is None:
        ...
        if src.dim() > 1:
            expanded = index.view(
                [-1 if i == dim else 1 for i in range(src.dim())]
            ).expand_as(src)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Add eps inside sqrt to prevent gradient explosion when mean is tiny
