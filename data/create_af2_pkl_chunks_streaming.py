...
def process_and_save_batch(cif_paths: List[Path], max_length: int, worker_id: int, temp_dir: Path, min_plddt: float = 70.0,
                           disable_tqdm: bool = True, fail_buffer_flush: int = 200, parse_timeout: int = 60) -> Tuple[str, int, int, str]:
    ...
    return str(temp_file), processed_count, failed_count, str(failed_log_path if failed_log_path.exists() else '')
        ...
        with ProcessPoolExecutor(max_workers=args.num_workers) as executor:
            ...
            if not args.no_global_tqdm:
                ...
                for future in tqdm(as_completed(futures), total=len(futures), desc="Processing batches"):
                    try:
                        temp_file, processed_count, failed_count, failed_log_path = future.result()
                        if temp_file:
