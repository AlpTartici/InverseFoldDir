...
from training.training_utils import mask_virtual_nodes_from_batch, apply_virtual_node_masking, apply_geometry_masking, apply_dssp_masking, apply_same_masking_to_weights, save_model_with_metadata, cleanup_old_checkpoints, get_rank, should_log_to_wandb, update_config_with_best_metrics, run_validation_phase, load_checkpoint_for_training, load_optimizer_and_scheduler_state
...
from training.distributed_utils import (
    ...
    create_distributed_sampler, cleanup_distributed, 
    is_main_process, reduce_loss_across_processes
)
        ...
        for p in group['params']:
            if p in optimizer.state:
                ...
                if target_device is not None and p.device != target_device:
                    ...
                    print(f"WARNING: Parameter device mismatch detected: {p.device} != {target_device}")
                ...
                if 'step' in state:
                    if torch.is_tensor(state['step']):
                        ...
                        state['step'] = torch.tensor(0, dtype=state['step'].dtype, device=state['step'].device)
                    else:
                        # Some use scalar steps
                        state['step'] = 0
                
                # Handle any other state tensors that might exist (comprehensive)
                for key, value in list(state.items()):  # Use list() to avoid dict modification during iteration
                    if torch.is_tensor(value) and key not in ['exp_avg', 'exp_avg_sq', 'step']:
                        if target_device is not None:
            ...
            for p in group['params']:
                if p in optimizer.state:
                    ...
                    for key, value in state.items():
                        if torch.is_tensor(value) and key != 'step':  # step can be on CPU
                            if value.device != target_device:
                                device_mismatches.append(f"Param {id(p)}, {key}: {value.device} != {target_device}")
        
        if device_mismatches:
            print(f"WARNING: Device mismatches after selective reset: {device_mismatches}")
            ...
            print(f"Device consistency verified in selective reset: all tensors on {target_device}")
...
def main():
    ...
    is_distributed, rank, world_size, local_rank, device = setup_distributed(args.device)
    print(f"DEBUG: Distributed setup complete. is_distributed={is_distributed}, rank={rank}, world_size={world_size}", flush=True)
    ...
    print(f"[DEBUG] Wandb setup: use_wandb={args.use_wandb}, is_smoke_test={is_smoke_test}, rank={current_rank}", flush=True)
    print(f"[DEBUG] should_log_to_wandb result: {should_log_to_wandb(args, is_smoke_test)}", flush=True)
    
    if should_log_to_wandb(args, is_smoke_test):
        print(f"[DEBUG] Attempting wandb initialization on rank {current_rank}", flush=True)
        try:
            import signal
            ...
            print(f"[DEBUG] wandb.login successful, attempting wandb.init with 120s timeout...", flush=True)
            ...
            try:
                ...
                print(f"Successfully initialized wandb in online mode (rank: {current_rank})", flush=True)
            except TimeoutError:
                signal.alarm(0)  # Cancel the alarm
                print(f"Wandb initialization timed out after 120 seconds, falling back to offline mode", flush=True)
            ...
            try:
                ...
                wandb.init(project=args.wandb_project, config=vars(args))
                print(f"Successfully initialized wandb in offline mode (rank: {current_rank})", flush=True)
        ...
        print(f"Skipping wandb initialization on rank {current_rank} (only rank 0 initializes wandb)", flush=True)
    ...
    if is_main_process():
        ...
        if args.ratio_af2_pdb == 0:
            ...
            print(f"Using homogeneous alternating training: {args.ratio_af2_pdb} pure AF2 batches per 1 pure PDB batch")
    ...
    graph_builder_kwargs = {
        ...
        'max_edge_dist': getattr(args, 'max_edge_dist', None),  # Distance-based edge building
    ...
    print(f"Creating unified dataloader (ratio_af2_pdb={args.ratio_af2_pdb})...", flush=True)
    ...
    train_loader = create_unified_dataloader(
        ...
        af2_chunk_limit=getattr(args, 'af2_chunk_limit', None),  # Limit chunks for testing
        # AF2 data is always loaded lazily - no upfront loading option
        
        # Mixing parameters
        ratio_af2_pdb=args.ratio_af2_pdb,
        heterogeneous_batches=getattr(args, 'heterogeneous_batches', True),  # Default to heterogeneous            
    ...
    if args.ratio_af2_pdb == -1:
        ...
        batch_type = "heterogeneous" if getattr(args, 'heterogeneous_batches', True) else "homogeneous"
        print(f"Mixed unified dataloader created ({args.ratio_af2_pdb} AF2 per 1 PDB, {batch_type} batches, deterministic iteration)")

    print(f"Training DataLoader created successfully! Starting validation dataset setup...", flush=True)
    ...
    if (args.split_json is not None) and (args.map_pkl is not None):
        if is_main_process():
            print(f"Creating PDB-only validation dataset using unified approach...", flush=True)
    ...
    if args.run_mode == 'overfit_on_one' and args.overfit_protein_name:
        ...
        try:
            ...
            print(f"[SUCCESS] Single protein dataset created: 1 protein only ({args.overfit_protein_name})")
        except ValueError as e:
            raise ValueError(f"Error creating single protein dataset: {e}. Check protein name and data availability.")
    
    # Create validation DataLoader
    if val_ds is not None:
        val_sampler = create_distributed_sampler(val_ds, is_distributed, shuffle=False) if is_distributed else None
    ...
    val_fixed_loader = DataLoader(val_ds, 
    ...
    val_unfixed_loader = DataLoader(val_ds, 
                           batch_size=args.batch,  # Original approach
                           shuffle=False,
                           sampler=val_sampler,               # Use distributed sampler if available
                           num_workers=args.num_workers,
    ...
    if args.run_mode == 'overfit':
        train_loader = val_fixed_loader  # Use fixed validation loader for training in overfit mode
    ...
    if args.run_mode == 'overfit_on_one':
        ...
        if args.overfit_protein_name and isinstance(val_ds, SingleProteinDataset):
            ...
            sequence_length = sample_data.num_nodes - (1 if getattr(sample_data, 'use_virtual_node', False) else 0)
            sample_idx = 0
            ...
            for idx in range(len(val_ds)):
                try:
                    ...
                    if sample_name == args.overfit_protein_name:
                        found_idx = idx
                        if is_main_process():
                            print(f"Found protein '{args.overfit_protein_name}' at index {idx}")
                        break
                except Exception as e:
                    if is_main_process():
            ...
            if found_idx is not None:
                ...
                if is_main_process():
                    print(f"Warning: Protein '{args.overfit_protein_name}' not found in validation set.")
        ...
        train_loader = DataLoader(single_sample, batch_size=1, shuffle=False, num_workers=0, collate_fn=collate_fn)
        val_fixed_loader = DataLoader(single_sample, batch_size=1, shuffle=False, num_workers=0, collate_fn=collate_fn)
        val_unfixed_loader = DataLoader(single_sample, batch_size=1, shuffle=False, num_workers=0, collate_fn=collate_fn)
        if is_main_process():
            print("Running in overfit_on_one mode: using single sample for both training and validation.")
            print(f"  Overfitting on protein: {protein_name} (source: {protein_source})")
    ...
    model = DFMNodeClassifier(
        gvp_kwargs=dict(
            node_dims=(args.node_dim_s, args.node_dim_v),  # Configurable node feature dimensions
            edge_dims=(args.edge_dim_s, args.edge_dim_v), # Configurable edge feature dimensions
            hidden_dims=(args.hidden_dim, args.hidden_dim_v),  # Use configurable hidden dimensions
        ...
        dfm_kwargs=dict(
            ...
            label_similarity_csv=args.label_similarity_csv if args.use_smoothed_labels else None
        ),
    ...
    if is_main_process():
        ...
        if frozen_params:
            print(f"  WARNING: {len(frozen_params)} parameters are frozen (requires_grad=False):")
    ...
    if is_main_process():
        ...
        print(f"  Total parameters: {total_params:,}")
        print(f"  Trainable parameters: {trainable_params:,}")
        print(f"  Model architecture: {args.num_layers_gvp} GVP layers, {args.num_layers_prediction} head layers, {args.hidden_dim} hidden dim, QKV={args.use_qkv}")


    # Optimizer Setup
    opt = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    scheduler = ReduceLROnPlateau(opt, mode='min', factor=0.25, patience=args.scheduler_patience)
    ...
    if hasattr(args, 'resume_from_checkpoint') and args.resume_from_checkpoint:
        ...
        try:
            ...
            if checkpoint_results['metrics']:
                best_val_loss = checkpoint_results['metrics'].get('best_val_loss', best_val_loss)
                best_val_accuracy = checkpoint_results['metrics'].get('best_val_accuracy', best_val_accuracy)  
                best_epoch = checkpoint_results['metrics'].get('best_epoch', best_epoch)
                print(f"Restored metrics - Best loss: {best_val_loss:.6f}, Best acc: {best_val_accuracy:.4f}, Best epoch: {best_epoch}")
                best_val_loss = best_val_loss * 100
                best_val_accuracy = best_val_accuracy / 10
                print(f"changed best_val_loss to {best_val_loss}, best_val_accuracy to {best_val_accuracy}", flush=True)
            ...
            if args.use_checkpoint_opt_and_sched:
                print(f"Optimizer loaded: {opt_scheduler_status.get('optimizer_loaded', False)}")

            # Check if learning rate was explicitly overridden from command line
            if args.use_checkpoint_opt_and_sched:
                lr_was_overridden = (
                    checkpoint_args_dict and 
                    'lr' in checkpoint_args_dict and 
                    'lr' in original_cmd_args_dict and
                    checkpoint_args_dict['lr'] != original_cmd_args_dict['lr'] and
                    opt_scheduler_status.get('optimizer_loaded', False)
                )
            ...
            if lr_was_overridden:
                ...
                current_optimizer_lrs = [param_group['lr'] for param_group in opt.param_groups]
                ...
                if hasattr(scheduler, 'cooldown_counter'):
                    ...
                    print(f"  Reset scheduler.cooldown_counter: {old_cooldown} -> {scheduler.cooldown_counter}")
                if hasattr(scheduler, 'num_bad_epochs'):
                    old_bad_epochs = scheduler.num_bad_epochs
                    scheduler.num_bad_epochs = 0
                    print(f"  Reset scheduler.num_bad_epochs: {old_bad_epochs} -> {scheduler.num_bad_epochs}")
                if hasattr(scheduler, '_last_lr'):
                    old_last_lr = scheduler._last_lr
                    scheduler._last_lr = [new_lr] * len(opt.param_groups)
                    print(f"  Reset scheduler._last_lr: {old_last_lr} -> {scheduler._last_lr}")
            ...
            if checkpoint_results['training_state']:
                ...
                epochs_since_lr_reduction = training_state.get('epochs_since_lr_reduction', 0)
                epochs_since_best_checkpoint = training_state.get('epochs_since_best_checkpoint', 0)
                ...
                print(f"Restored training state - LR: {current_lr}, patience: {patience_counter}")
    ...
    if args.enable_checkpoint_rollback:
        rollback_patience_threshold = int(scheduler.patience * args.rollback_patience_factor)
    ...
    if hasattr(args, 'sampling_only') and args.sampling_only:
        if not args.external_checkpoint:
            raise ValueError("sampling_only mode requires --external_checkpoint to be specified")
        
        print("="*60)
        print("SAMPLING-ONLY MODE DETECTED")
        print("="*60)
        print(f"Skipping training, using external checkpoint: {args.external_checkpoint}")
    ...
    if args.max_batches_per_epoch is not None:
        ...
        if is_main_process():
            print(f"Using user-specified batch limit: {max_batches_per_epoch} batches per epoch")
    else:
        # Use the train_loader dataset length for max batches
        max_batches_per_epoch = len(train_loader) if hasattr(train_loader, '__len__') else float('inf')
        if is_main_process():
            print(f"Using full dataset: {max_batches_per_epoch} batches per epoch")
    
    # Create epoch progress bar (only on main process)
    if is_main_process():
        epoch_pbar = tqdm(range(start_epoch, max_epochs + 1), desc="Training", unit="epoch", 
                          disable=is_smoke_test, leave=True, ncols=100)
    else:
    ...
    val_fixed_gen = torch.Generator(device=device).manual_seed(FIXED_SEED_FOR_VAL_FIXED_NOISE)
    ...
    try:
        for ep in epoch_pbar:
            ...
            epoch_predicted_classes = []  # Collect all predictions for diversity analysis
            ...
            if args.verbose:
                ...
                print(f"[DEBUG] EPOCH {ep}: Reset dataset-level global protein tracker for {args.num_workers} workers")
            
            # Debug: Print verbose status at epoch start
            if ep == 1:  # Only print once
                print(f"[DEBUG] EPOCH START: args.verbose={getattr(args, 'verbose', 'NOT_SET')}, is_main_process()={is_main_process()}")
            ...
            if args.ratio_af2_pdb != 0 and max_batches_per_epoch < 5 and is_main_process():
                hybrid_batch_stats = []
            ...
            for batch_idx, batch in batch_iterator:
                ...
                if len(batch) == 5:
                    ...
                    if time_values[0] is not None:
                        t = torch.tensor([tv for tv in time_values if tv is not None], device=device)
                        B = t.shape[0]
                    else:
                        raise Exception("None observed in time values. Check get_item, the collator etc to make sure the time gets sampled and passed correctly.")
                elif len(batch) == 4:
                    if args.lambda_dssp_loss is not None:
                        if args.lambda_dssp_loss > 0:
                            raise Exception("DSSP targets are required but not provided.")
                    ...
                    raise Exception("Time values are missing. They should've been generated inside __get_item__() and passed to the DataLoader.")
                ...
                if epoch_timeout != float('inf'):
                    ...
                    if elapsed_time > epoch_timeout:
                        if is_main_process():
                            print(f"Epoch timeout reached after {elapsed_time/60:.1f} minutes (limit: {epoch_timeout/60:.1f} minutes)")
                ...
                if args.ratio_af2_pdb != 0 and max_batches_per_epoch < 5 and is_main_process():
                    ...
                    print(f"Hybrid Batch {batch_idx + 1}/{max_batches_per_epoch}: {B} proteins, avg length: {avg_seq_len:.1f}, total nodes: {num_nodes}", flush=True)
                ...
                if args.verbose and is_main_process():
                    ...
                    shared_epoch_count, shared_cumulative_count = get_global_protein_counts()
                    
                    # Debug logging for first few batches
                    if batch_idx < 10:
                        print(f"[DEBUG] UNION ACROSS ALL {args.num_workers} WORKERS - Batch {batch_idx}: epoch_unique={shared_epoch_count}, cumulative={shared_cumulative_count}")
                ...
                if t is not None:
                    ...
                    t = t.to(device)
                    if args.verbose:
                        print(f"Using dataset-sampled time values: mean={t.mean():.3f}, std={t.std():.3f}")
                else:
                    raise Exception("t is none. __get_item__ didn't create it correctly or it wasn't passed correctly.")
                ...
                if args.run_mode == 'overfit_on_one':
                    x_t, max_train_dirichlet_prob = sample_forward(y, t, generator=None, return_max_prob=True, 
                                                                 dirichlet_multiplier=dirichlet_multiplier)
                    # Log the max probability to monitor noise variation in overfit mode
                    if should_log_to_wandb(args, is_smoke_test):
                        wandb.log({f"train/max_dirichlet_prob": max_train_dirichlet_prob})

                else:
                    x_t = sample_forward(y, t, generator=None, dirichlet_multiplier=dirichlet_multiplier)  # Use global RNG for training diversity
                ...
                try:
                    ...
                    if is_main_process():
                        ...
                        print(f"[WARNING] Skipping this batch and continuing training...")
                        print(f"[WARNING] Total OOM batches skipped so far: {oom_skip_count}/{total_batches_attempted}")
                        
                        # Log memory stats for debugging
                        if torch.cuda.is_available():
                            allocated = torch.cuda.memory_allocated() / (1024**3)  # GB
                            reserved = torch.cuda.memory_reserved() / (1024**3)    # GB
                            print(f"[WARNING] GPU Memory - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB")
                    ...
                    if should_log_to_wandb(args, is_smoke_test):
                        wandb.log({
                            ...
                            'train/oom_skip_rate': oom_skip_count / total_batches_attempted,
                            'epoch': ep,
                            'batch': batch_idx
                        })
                ...
                if args.use_virtual_node:
                    ...
                    position_logits_masked, y_masked = apply_geometry_masking(position_logits, y, data, B, K, device, args.use_virtual_node, v_pred_is_graph_space=True)
                    ...
                    if args.flexible_loss_scaling:
                        ...
                        if args.debug_mode and ep == 1 and batch_idx == 0:
                            ...
                            print(f"    Sample values (first 20): {uncertainty_weights[:20].cpu().numpy()}")
                            ...
                            if uncertainty_weights_masked is not None:
                                ...
                                print(f"    Mean: {uncertainty_weights_masked.mean().item():.6f}")
                                print(f"    Std: {uncertainty_weights_masked.std().item():.6f}")
                                print(f"    Min: {uncertainty_weights_masked.min().item():.6f}")
                                print(f"    Max: {uncertainty_weights_masked.max().item():.6f}")
                                print(f"    Sample values (first 20): {uncertainty_weights_masked[:20].cpu().numpy()}")
                                
                                # Analyze weight distribution
                                weights_np = uncertainty_weights_masked.cpu().numpy()
                                center_count = ((weights_np > 0.4) & (weights_np < 0.6)).sum()
                                total_count = len(weights_np)
                                center_pct = center_count / total_count * 100
                                print(f"  Weight distribution analysis:")
                                print(f"    Weights in [0.4, 0.6]: {center_count}/{total_count} ({center_pct:.1f}%)")
                                print(f"    Coefficient of variation: {uncertainty_weights_masked.std().item() / uncertainty_weights_masked.mean().item():.3f}")
                            print(f"  End flexible loss scaling debug\n")
                else:
                    
                    # No virtual nodes: still apply geometry masking for nodes with missing coordinates
                    # position_logits comes from model output and is already in graph space
                    position_logits_masked, y_masked = apply_geometry_masking(position_logits, y, data, B, K, device, use_virtual_node=False, v_pred_is_graph_space=True)
                ...
                if args.flexible_loss_scaling and uncertainty_weights_masked is not None:
                    ...
                    if args.use_smoothed_labels:
                        ...
                        log_probs = F.log_softmax(position_logits_masked.view(-1, K), dim=-1)  # [num_valid_nodes, K]
                        ...
                        cce_loss = weighted_loss.sum() / total_weights if total_weights > 0 else per_sample_loss.mean()
                    else:
                        # Hard labels: use standard cross-entropy weighted by uncertainty
                        targets = y_masked.argmax(dim=-1)  # [num_valid_nodes]
                        per_sample_loss = F.cross_entropy(position_logits_masked.view(-1, K), targets, reduction='none')
                        weighted_loss = per_sample_loss * uncertainty_weights_masked
                        total_weights = uncertainty_weights_masked.sum()
                        cce_loss = weighted_loss.sum() / total_weights if total_weights > 0 else per_sample_loss.mean()
                    ...
                    if args.debug_mode and ep == 1 and batch_idx == 0:
                        ...
                        if args.use_smoothed_labels:
                            ...
                            standard_loss = F.cross_entropy(position_logits_masked.view(-1, K), targets, reduction='mean')
                        
                        print(f"\nLOSS COMPUTATION COMPARISON:")
                        print(f"  Standard loss (unweighted): {standard_loss.item():.6f}")
                        print(f"  Flexible loss (weighted):   {cce_loss.item():.6f}")
                        print(f"  Absolute difference:        {abs(standard_loss.item() - cce_loss.item()):.6f}")
                        print(f"  Relative difference:        {abs(standard_loss.item() - cce_loss.item()) / standard_loss.item() * 100:.4f}%")
                        print(f"  Total weight sum:           {total_weights.item():.6f}")
                        print(f"  Average weight:             {uncertainty_weights_masked.mean().item():.6f}")
                    ...
                    cce_loss = cond_flow.cross_entropy_loss(
                        position_logits_masked.view(-1, K),     # [num_valid_nodes, K] - raw logits
                ...
                if args.lambda_dssp_loss > 0.0 and dssp_targets is not None and dssp_logits is not None:
                    ...
                    if hasattr(model_obj, 'dssp_final'):
                        ...
                        for i, target in enumerate(dssp_targets):
                            if target is not None:
                                ...
                                if hasattr(mask, '__len__') and len(mask) > i:
                                    dummy_len = mask[i].sum().item()  # Use actual sequence length from mask
                                    ...
                                    dummy_len = position_logits.shape[0] // len(dssp_targets)
                                ...
                                dummy_targets = torch.zeros(dummy_len, dtype=torch.long, device=device)
                        ...
                        if dssp_targets_concat is not None:
                            ...
                            dssp_logits_masked, dssp_targets_masked = apply_dssp_masking(
                                dssp_logits, dssp_targets_concat, data, B, device, args.use_virtual_node, debug=args.debug_mode
                            )
                            ...
                            if args.debug_mode and ep == 1 and batch_idx == 0:
                                print(f"\nDSSP LOSS DEBUG:")
                                print(f"  DSSP targets in batch: {sum(1 for t in dssp_targets if t is not None)}/{len(dssp_targets)}")
                                print(f"  DSSP logits shape: {dssp_logits.shape}")
                                print(f"  DSSP targets concat shape: {dssp_targets_concat.shape}")
                                print(f"  DSSP logits masked shape: {dssp_logits_masked.shape}")
                                print(f"  DSSP targets masked shape: {dssp_targets_masked.shape}")
                        ...
                        print(f"WARNING: DSSP loss enabled but model has no dssp_final layer")
                        dssp_loss = torch.tensor(0.0, device=device)
                        dssp_accuracy = 0.0
                else:
                    dssp_loss = torch.tensor(0.0, device=device)
                ...
                if position_logits_masked.numel() == 0:
                    print(f"      ERROR: all data masked out! position_logits_masked is empty")
                    print(f"      Debug info:")
                    ...
                    if args.use_virtual_node:
                        real_node_mask = mask_virtual_nodes_from_batch(data, B)
                        print(f"        virtual nodes: {(~real_node_mask).sum().item()} out of {real_node_mask.size(0)}")
                    if hasattr(data, 'geom_missing') and data.geom_missing is not None:
                        print(f"        geometry missing nodes: {data.geom_missing.sum().item()}")
                    
                    # This should not happen - there's a bug in masking logic
                    raise ValueError("All data was masked out - this indicates a bug in the masking logic")
                ...
                predicted_classes = position_logits_masked.argmax(dim=-1)  # [total_real_nodes]
                ...
                train_accuracy = correct_predictions.mean().item()  # Average accuracy for this batch
                ...
                if hasattr(args, 'grad_clip') and args.grad_clip > 0:
                    ...
                    if batch_idx < 3:
                        print(f"    Gradient clipping applied: norm={grad_norm:.6f} (max_norm={args.grad_clip})")
                    ...
                    if should_log_to_wandb(args, is_smoke_test) and args.run_mode == 'overfit_on_one':
                ...
                should_log_gradient = should_log_to_wandb(args, is_smoke_test) and args.run_mode == 'overfit_on_one'
                ...
                if is_main_process() and args.verbose:
                    log_msg = (f"Epoch {ep} Batch {batch_idx + 1}: loss={total_loss.item():.6f}, "
                ...
                if should_log_to_wandb(args, is_smoke_test):
                    ...
                    if args.verbose and is_main_process():
                        protein_count = B  # Use batch size since we track proteins at dataset level
                        
                        # Use coordinated counts if available, otherwise use 0 (dataset-level tracking handles this)
                        if 'local_epoch_unique_count' in locals() and 'local_cumulative_unique_count' in locals():
                        ...
                        # Debug logging for first few batches
                        if batch_idx < 3:
                            mode = f"UNION-ACROSS-{args.num_workers}-WORKERS" if args.num_workers > 1 else "SINGLE-WORKER"
                            print(f"[DEBUG] {mode} WandB logging: epoch_unique={epoch_count_to_log}, "
                                  f"cumulative={cumulative_count_to_log}, batch_count={protein_count}")
                            print(f"[DEBUG] Using dataset-level protein tracking, batch_count=B={B}")
                            print(f"[DEBUG] wandb_batch_dict keys: {list(wandb_batch_dict.keys())}")
                    
                    wandb.log(wandb_batch_dict)
                
                # Additional dedicated logging for unique protein tracking (verbose mode)
                if args.verbose and is_main_process() and should_log_to_wandb(args, is_smoke_test):
                    protein_count = B  # Use batch size since we track proteins at dataset level
                    ...
                    if batch_idx < 3:
                        print(f"[DEBUG] Logging unique proteins to WandB - args.verbose={args.verbose}, "
                              f"is_main_process={is_main_process()}, should_log={should_log_to_wandb(args, is_smoke_test)}")
                        print(f"[DEBUG] wandb module available: {'wandb' in globals()}, "
                    ...
                    try:
                        ...
                        if batch_idx < 3:
                            print(f"[DEBUG] Successfully logged to wandb: epoch_unique={epoch_count_to_log}, "
            ...
            avg_train_accuracy = total_train_accuracy / len(train_loader) if len(train_loader) > 0 else 0
            
            # Compute average gradient norms for the epoch
            avg_gradient_norm = total_gradient_norm / gradient_count if gradient_count > 0 else 0
            
            # Report OOM batch skip statistics for this epoch
            if is_main_process() and (oom_skip_count > 0 or total_batches_attempted > 0):
                oom_skip_rate = oom_skip_count / total_batches_attempted if total_batches_attempted > 0 else 0
                ...
                print(f"[INFO] - Successfully processed batches: {total_batches_attempted - oom_skip_count}")
            ...
            avg_gradient_norm = total_gradient_norm / gradient_count if gradient_count > 0 else 0.0 
            ...
            if epoch_predicted_classes:
                ...
                epoch_pred_counts = torch.bincount(all_predictions, minlength=21)  # 21 amino acids (20 standard + X)
                epoch_most_frequent_ratio = epoch_pred_counts.max().item() / all_predictions.numel()
                epoch_avg_entropy = np.mean(epoch_pred_entropies) if epoch_pred_entropies else 0.0
                
                
            else:
                raise ValueError("No predictions collected during training epoch - this indicates a bug in the training loop.")
            ...
            avg_val_fixed_loss = val_t0_metrics['avg_loss']  # Use t=0 validation as primary fixed validation
            avg_val_fixed_accuracy = val_t0_metrics['avg_accuracy']
            val_fixed_diversity_metrics = val_t0_metrics['diversity_metrics']  # For backward compatibility
            val_t0_diversity_metrics = val_t0_metrics['diversity_metrics']  # For console logging
            ...
            val_unfixed_epoch_avg_metrics = val_unfixed_metrics.get('epoch_avg_metrics', {})
            
            # Extract DSSP metrics for val_unfixed
            avg_val_unfixed_dssp_loss = val_unfixed_metrics.get('avg_dssp_loss', 0.0)
            avg_val_unfixed_dssp_accuracy = val_unfixed_metrics.get('avg_dssp_accuracy', 0.0)
            ...
            avg_val_combined_accuracy = (avg_val_fixed_accuracy + avg_val_unfixed_accuracy) / 2.0
            
            # Calculate comprehensive validation metric (average of all 5 validation losses)
            avg_val_all_loss = (avg_val_fixed_loss + avg_val_unfixed_loss + avg_val_t2_loss + avg_val_t4_loss + avg_val_t6_loss) / 5.0
            avg_val_all_accuracy = (avg_val_fixed_accuracy + avg_val_unfixed_accuracy + avg_val_t2_accuracy + avg_val_t4_accuracy + avg_val_t6_accuracy) / 5.0
            
            # Select validation metric based on configuration
            def get_validation_metric(args, train_loss, val_fixed_loss, val_unfixed_loss, val_combo_loss, val_all_loss):
            ...
            primary_val_loss, primary_val_name = get_validation_metric(
                args, avg_train_loss, avg_val_fixed_loss, avg_val_unfixed_loss, avg_val_combined_loss, avg_val_all_loss
            )
            ...
            if args.val_metric == 'training':
                ...
                primary_val_accuracy = (avg_val_fixed_accuracy + avg_val_unfixed_accuracy) / 2.0
            elif args.val_metric == 'val_all':
            ...
            if should_print:
                ...
                tqdm.write(f"      val_fixed   - Loss: {avg_val_fixed_loss:.6f}, Acc: {avg_val_fixed_accuracy:.4f}")
                tqdm.write(f"      val_unfixed - Loss: {avg_val_unfixed_loss:.6f}, Acc: {avg_val_unfixed_accuracy:.4f}")
                tqdm.write(f"      val_t2      - Loss: {avg_val_t2_loss:.6f}, Acc: {avg_val_t2_accuracy:.4f}")
                tqdm.write(f"      val_t4      - Loss: {avg_val_t4_loss:.6f}, Acc: {avg_val_t4_accuracy:.4f}")
                tqdm.write(f"      val_t6      - Loss: {avg_val_t6_loss:.6f}, Acc: {avg_val_t6_accuracy:.4f}")
                tqdm.write(f"      val_combined- Loss: {avg_val_combined_loss:.6f}, Acc: {avg_val_combined_accuracy:.4f}")
                tqdm.write(f"      val_all     - Loss: {avg_val_all_loss:.6f}, Acc: {avg_val_all_accuracy:.4f}")
                tqdm.write(f"      PRIMARY ({primary_val_name}): {primary_val_loss:.6f} [used for LR/early stopping]")
                tqdm.write(f"      Train/val_fixed gap: {train_val_fixed_gap:.6f}")
                tqdm.write(f"      Train/val_unfixed gap: {train_val_unfixed_gap:.6f}")
                tqdm.write(f"      val_fixed/val_unfixed gap: {val_fixed_unfixed_gap:.6f}")
                # Warn if significant discrepancy suggests regularization issues
                if train_val_unfixed_gap > 1.5 * train_val_fixed_gap and not args.run_mode == 'overfit_on_one':
                    tqdm.write(f"      WARNING: Large train/val_unfixed gap suggests potential regularization issues!")
                if val_fixed_unfixed_gap > 0.1 and not args.run_mode == 'overfit_on_one':
                    tqdm.write(f"      INFO: val_fixed vs val_unfixed gap indicates noise schedule sensitivity.")
            ...
            if args.enable_checkpoint_rollback:
                ...
                if current_lr < args.min_lr_stop_point:
                    print(f"Learning rate dropped to {current_lr:.2e}, which is below minimum threshold of {args.min_lr_stop_point:.2e}.")
                    print("Early stopping triggered due to minimum learning rate threshold.")
                    break
                ...
                if current_lr < old_lr:
                    ...
                    print(f"  Starting rollback monitoring (threshold: {rollback_patience_threshold} epochs)")
                else:
            ...
            if is_main_process():
                print(f"DEBUG: Epoch {ep} - should_log_to_wandb={should_log}, args.use_wandb={args.use_wandb}, is_smoke_test={is_smoke_test}, run_mode={args.run_mode}")
            ...
            if should_log:
                ...
                wandb_log_dict = {
                    ...
                    'train/successful_batches': total_batches_attempted - oom_skip_count,
                    'train/oom_skip_rate': oom_skip_count / total_batches_attempted if total_batches_attempted > 0 else 0,
                        ...
                        'val_fixed/epoch_unique_predictions': val_fixed_diversity_metrics['unique_predictions'],
                        'val_fixed/epoch_most_frequent_ratio': val_fixed_diversity_metrics['most_frequent_ratio'],
                        'val_fixed/epoch_avg_entropy': val_fixed_diversity_metrics['avg_entropy'],
                        ...
                        'val_t2/epoch_unique_predictions': val_t2_diversity_metrics['unique_predictions'],
                        'val_t2/epoch_most_frequent_ratio': val_t2_diversity_metrics['most_frequent_ratio'],
                        ...
                        'val_t4/epoch_unique_predictions': val_t4_diversity_metrics['unique_predictions'],
                        'val_t4/epoch_most_frequent_ratio': val_t4_diversity_metrics['most_frequent_ratio'],
                        ...
                        'val_t6/epoch_unique_predictions': val_t6_diversity_metrics['unique_predictions'],
                        'val_t6/epoch_most_frequent_ratio': val_t6_diversity_metrics['most_frequent_ratio'],
                        ...
                        'val_unfixed/epoch_unique_predictions': val_unfixed_diversity_metrics['unique_predictions'],
                        'val_unfixed/epoch_most_frequent_ratio': val_unfixed_diversity_metrics['most_frequent_ratio'],
                        'val_unfixed/epoch_avg_entropy': val_unfixed_diversity_metrics['avg_entropy'],
                ...
                if 'avg_time_sampled' in val_fixed_epoch_avg_metrics:
                    wandb_log_dict['val_fixed/epoch_avg_time_sampled'] = val_fixed_epoch_avg_metrics['avg_time_sampled']
                if 'avg_max_dirichlet_prob' in val_fixed_epoch_avg_metrics:
                    wandb_log_dict['val_fixed/epoch_avg_max_dirichlet_prob'] = val_fixed_epoch_avg_metrics['avg_max_dirichlet_prob']
                if 'avg_max_dirichlet_prob' in val_unfixed_epoch_avg_metrics:
                    wandb_log_dict['val_unfixed/epoch_avg_max_dirichlet_prob'] = val_unfixed_epoch_avg_metrics['avg_max_dirichlet_prob']
                
                # Add unique protein tracking metrics (verbose mode only)
                if args.verbose and is_main_process():
                    # Get current counts from dataset-level tracking
                    from data.unified_dataset import get_global_protein_counts
                    dataset_epoch_count, dataset_cumulative_count = get_global_protein_counts()
            ...
            if is_main_process():
                ...
                tqdm.write(f"Epoch {ep}/{max_epochs} (Duration: {epoch_duration_minutes:.2f} min)")
                tqdm.write(f"  Train      - Total: {avg_train_loss:.4f}, Acc: {avg_train_accuracy:.4f}")
                tqdm.write(f"  Val t=0    - Total: {val_t0_metrics['avg_loss']:.4f}, Acc: {val_t0_metrics['avg_accuracy']:.4f}")
                tqdm.write(f"  Val t=2    - Total: {val_t2_metrics['avg_loss']:.4f}, Acc: {val_t2_metrics['avg_accuracy']:.4f}")
                tqdm.write(f"  Val t=4    - Total: {val_t4_metrics['avg_loss']:.4f}, Acc: {val_t4_metrics['avg_accuracy']:.4f}")
                tqdm.write(f"  Val t=6    - Total: {val_t6_metrics['avg_loss']:.4f}, Acc: {val_t6_metrics['avg_accuracy']:.4f}")
                tqdm.write(f"  Val Unfixed- Total: {avg_val_unfixed_loss:.4f}, Acc: {avg_val_unfixed_accuracy:.4f}")
                tqdm.write(f"  Train Diversity - Unique: {epoch_unique_predictions}/20, MostFreq: {epoch_most_frequent_ratio:.3f}, Entropy: {epoch_avg_entropy:.3f}")
                ...
                if args.verbose:
                    ...
                    if 'local_epoch_unique_count' in locals() and 'local_cumulative_unique_count' in locals():
                        ...
                        from data.unified_dataset import get_global_protein_counts
                        epoch_count_to_display, cumulative_count_to_display = get_global_protein_counts()
                    
                    tqdm.write(f"  Protein Union (All {args.num_workers} Workers) - Epoch: {epoch_count_to_display} unique, Cumulative: {cumulative_count_to_display} unique")
                
                tqdm.write(f"  Val t=0 Div     - Unique: {val_t0_diversity_metrics['unique_predictions']}/20, MostFreq: {val_t0_diversity_metrics['most_frequent_ratio']:.3f}, Entropy: {val_t0_diversity_metrics['avg_entropy']:.3f}")
                tqdm.write(f"  Val Unfixed Div - Unique: {val_unfixed_diversity_metrics['unique_predictions']}/20, MostFreq: {val_unfixed_diversity_metrics['most_frequent_ratio']:.3f}, Entropy: {val_unfixed_diversity_metrics['avg_entropy']:.3f}")
                ...
                if 'top_predicted_aa' in val_t0_diversity_metrics and 'bottom_predicted_aa' in val_t0_diversity_metrics:
                    # Top 3 most predicted amino acids (val_t0)
                    top_aa_strs = [f"{aa}({count}, {ratio:.1%})" for aa, count, ratio in val_t0_diversity_metrics['top_predicted_aa']]
                    ...
                    if val_t0_diversity_metrics['bottom_predicted_aa']:
                        bottom_aa_strs = [f"{aa}({count}, {ratio:.1%})" for aa, count, ratio in val_t0_diversity_metrics['bottom_predicted_aa']]
                        print(f"  Val t=0 Bottom 3 AA - {', '.join(bottom_aa_strs)}")
                    else:
                        print(f"  Val t=0 Bottom 3 AA - No predictions for least frequent amino acids")
            ...
            if is_distributed:
                ...
                val_t0_loss_tensor = torch.tensor(val_t0_metrics['avg_loss'], device=device)
                val_t0_metrics['avg_loss'] = reduce_loss_across_processes(val_t0_loss_tensor).item()
                
                val_t2_loss_tensor = torch.tensor(val_t2_metrics['avg_loss'], device=device)
                val_t2_metrics['avg_loss'] = reduce_loss_across_processes(val_t2_loss_tensor).item()
                
                val_t4_loss_tensor = torch.tensor(val_t4_metrics['avg_loss'], device=device)
                val_t4_metrics['avg_loss'] = reduce_loss_across_processes(val_t4_loss_tensor).item()
                
                val_t6_loss_tensor = torch.tensor(val_t6_metrics['avg_loss'], device=device)
                val_t6_metrics['avg_loss'] = reduce_loss_across_processes(val_t6_loss_tensor).item()
                
                val_unfixed_tensor = torch.tensor(avg_val_unfixed_loss, device=device)
                avg_val_unfixed_loss = reduce_loss_across_processes(val_unfixed_tensor).item()
                ...
                avg_val_fixed_loss = val_t0_metrics['avg_loss']  # For backward compatibility
                avg_val_combined_loss = (avg_val_fixed_loss + avg_val_unfixed_loss) / 2.0
                # Recalculate val_all_loss with aggregated values
                avg_val_all_loss = (avg_val_fixed_loss + avg_val_unfixed_loss + val_t2_metrics['avg_loss'] + val_t4_metrics['avg_loss'] + val_t6_metrics['avg_loss']) / 5.0
                primary_val_loss, primary_val_name = get_validation_metric(args, avg_train_loss, avg_val_fixed_loss, avg_val_unfixed_loss, avg_val_combined_loss, avg_val_all_loss)
            ...
            if primary_val_loss < best_val_loss:
                ...
                if is_main_process():
                    ...
                    try:
                        ...
                        if hasattr(args, 'output_dir') and args.output_dir:
                            try:
                                import shutil
                                output_dir_models = os.path.join(args.output_dir, 'saved_models')
                                os.makedirs(output_dir_models, exist_ok=True)
                                
                                # Copy best_model.pt for easy reference (gets overwritten)
                                best_model_src = os.path.join(output_base, 'saved_models', 'best_model.pt')
                                best_model_dst = os.path.join(output_dir_models, 'best_model.pt')
                                if os.path.exists(best_model_src):
                                    shutil.copy2(best_model_src, best_model_dst)
                                    print(f"  Best model also saved to output-dir: {best_model_dst}")
                                
                                # Copy historical best model (never overwritten)
                                historical_filename = f"best_upto_epoch_{ep}.pt"
                                historical_src = os.path.join(output_base, 'saved_models', historical_filename)
                                historical_dst = os.path.join(output_dir_models, historical_filename)
                                if os.path.exists(historical_src):
                                    shutil.copy2(historical_src, historical_dst)
                                    print(f"  Historical best model also saved to output-dir: {historical_dst}")
                                
                                # Copy timestamped version too
                                checkpoint_filename = os.path.basename(best_model_path)
                                if checkpoint_filename not in ['best_model.pt', historical_filename]:  # Don't copy thrice
                                    output_checkpoint_path = os.path.join(output_dir_models, checkpoint_filename)
                                    shutil.copy2(best_model_path, output_checkpoint_path)
                                    print(f"  Timestamped best model also saved to output-dir: {output_checkpoint_path}")
                            except Exception as copy_e:
                                print(f"  Warning: Failed to copy best model to output-dir: {copy_e}")
                        ...
                        if args.checkpoint_copy_dir and is_main_process():
                            try:
                                ...
                                # Copy best_model.pt for easy reference (gets overwritten)
                                best_model_src = os.path.join(output_base, 'saved_models', 'best_model.pt')
                                best_model_dst = os.path.join(args.checkpoint_copy_dir, 'best_model.pt')
                                if os.path.exists(best_model_src):
                                    shutil.copy2(best_model_src, best_model_dst)
                                    print(f"  Best model copied to remote location: {best_model_dst}")
                                
                                # Copy historical best model (never overwritten)
                                historical_filename = f"best_upto_epoch_{ep}.pt"
                                historical_src = os.path.join(output_base, 'saved_models', historical_filename)
                                historical_dst = os.path.join(args.checkpoint_copy_dir, historical_filename)
                                if os.path.exists(historical_src):
                                    shutil.copy2(historical_src, historical_dst)
                                    print(f"  Historical best model copied to remote location: {historical_dst}")
                                
                                # Copy timestamped version too
                                checkpoint_filename = os.path.basename(best_model_path)
                                if checkpoint_filename not in ['best_model.pt', historical_filename]:  # Don't copy thrice
                                    remote_checkpoint_path = os.path.join(args.checkpoint_copy_dir, checkpoint_filename)
                                    shutil.copy2(best_model_path, remote_checkpoint_path)
                                    print(f"  Timestamped best model copied to remote location: {remote_checkpoint_path}")
                                    
                            except Exception as copy_e:
                                print(f"  Warning: Failed to copy best model to remote location: {copy_e}")
                        
                        # Clean up old checkpoints
                        if args.save_intermediate_models:
                            cleanup_old_checkpoints(output_base, model_name, keep_last_n=5)
                            
                    except Exception as e:
                        print(f"Error saving model with metadata: {e}")
                        # Fallback to simple save
                        try:
                            model_save_path = os.path.join(output_base, 'saved_models', 'best_model.pt')
                            torch.save(model, model_save_path)
                        except Exception as e2:
                            print(f"Error with fallback save: {e2}")
                            torch.save(model, 'best_model.pt')  # Final fallback
                    
                    print(f"  New best model saved ({primary_val_name}: {best_val_loss:.4f})")
            else:
                patience_counter += 1
                ...
                if (args.enable_checkpoint_rollback and 
                    best_model_state is not None and 
                    lr_has_dropped and  # Only allow rollback after LR drop
                    epochs_since_lr_reduction >= rollback_patience_threshold and 
                    epochs_since_best_checkpoint >= rollback_patience_threshold):
                    
                    if is_main_process():
                        print(f"   CHECKPOINT ROLLBACK: No improvement for {epochs_since_best_checkpoint} epochs since LR reduction")
                        print(f"     Restoring model to best checkpoint (val_loss: {best_val_loss:.4f})")
                    
                    # Restore model parameters on all ranks
                    model_device = next(model.parameters()).device
                    model.load_state_dict({k: v.to(model_device) for k, v in best_model_state.items()})
                    
                    # Selective optimizer reset: reset momentum but keep adaptive scaling
                    if is_main_process():
                        print(f"     Resetting optimizer momentum while preserving adaptive scaling for LR={current_lr:.2e}")
                    ...
                    reset_optimizer_momentum_selectively(opt, target_device=model_device)
                                        ...
                                        for key, value in state.items():
                                            if torch.is_tensor(value) and key != 'step':  # step can be on CPU
                                                if value.device != model_device:
                                                    device_mismatches.append(f"{key}: {value.device} != {model_device}")
                            
                            if device_mismatches:
                                print(f"     WARNING: Device mismatches found after rollback: {device_mismatches}")
                            else:
                                print(f"     Device consistency verified: all optimizer tensors on {model_device}")
                        except Exception as e:
                            print(f"     Device verification failed (non-critical): {e}")
                ...
                # Do not reset epochs_since_lr_reduction - that tracks actual LR drops, not rollbacks
                    patience_counter = 0  # Give the model another chance with the restored state
                    
                    # Increment rollback counter
                    rollback_count += 1
                    
                    if is_main_process():
                        print(f"     Rollback complete, continuing with LR={current_lr:.2e}")
                        print(f"     Total rollbacks so far: {rollback_count}")
                        print(f"     Next rollback possible after {rollback_patience_threshold} epochs without LR reduction")
        ...
        if is_main_process():
            ...
            update_config_with_best_metrics(config_dir, model_name, best_metrics, job_timestamp)
