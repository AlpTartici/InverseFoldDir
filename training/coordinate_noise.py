...
def apply_coordinate_noise_to_batch(
    data: Union[Data, Batch], t: torch.Tensor, args
) -> Union[Data, Batch]:
    """
    ...
    if isinstance(data, Batch):
        ...
        if len(t) != batch_size:
            raise ValueError(
                f"Time tensor size {len(t)} doesn't match batch size {batch_size}"
            )
        ...
        if len(t) != 1:
            raise ValueError(
                f"Expected single time value for single Data object, got {len(t)}"
            )
...
def apply_noise_to_sample(data: Data, start_idx: int, end_idx: int, t: float, args):
    ...
    geom_missing = (
        data.geom_missing[start_idx:end_idx]
        if hasattr(data, "geom_missing")
    ...
    if args.uncertainty_struct_noise_scaling:
        ...
        uncert_coeff = uncert_coeff.unsqueeze(1).unsqueeze(
            2
        )  # [L, 1, 1] for broadcasting
    else:
        uncert_coeff = 1.0
...
def rebuild_geometric_features_after_noise(
    data: Union[Data, Batch], graph_builder
) -> Union[Data, Batch]:
    """
