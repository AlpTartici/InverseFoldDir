...
class BatchStructurePredictor:
    ...
    def __init__(self, csv_path: str, output_dir: str, reference_dir: str, verbose: bool = False,
        ...
        if verbose:
            ...
            if self.reference_dir.exists():
                ...
                if len(ref_files) > 0:
                    logger.debug(f"  Sample reference files: {[f.name for f in ref_files[:3]]}")
            else:
    ...
    def load_predictions(self) -> pd.DataFrame:
        ...
        try:
            ...
            def convert_sequence_format(row):
                ...
                if isinstance(pred_seq, str) and pred_seq.startswith('[') and pred_seq.endswith(']'):
                    try:
                        ...
                        logger.info(f"Converted sequence for {row['structure_name']}: {len(indices)} indices -> {len(aa_sequence)} amino acids")
                        return aa_sequence
                        
                    except (ValueError, SyntaxError) as e:
                        logger.warning(f"Failed to parse indices for {row['structure_name']}: {e}")
                        return pred_seq
            ...
            def check_length_consistency(row):
                ...
                if 'true_sequence' in df.columns:
                    ...
                    if pd.isna(true_seq) or true_seq is None or true_seq == '':
                        logger.debug(f"No true_sequence available for {row['structure_name']} (evaluation will proceed)")
                    else:
            ...
            logger.info(f"Sequence length range: {df['length'].min()}-{df['length'].max()} amino acids")
    ...
    def predict_batch(self, overwrite: bool = False) -> Dict[str, Any]:
        ...
        if self.protein_subset:
            ...
            logger.info(f"Filtered dataset: {len(df)} proteins from subset (was {initial_count} total)")
        ...
        if self.verbose:
            ...
            if self.protein_subset:
                logger.debug(f"  Using protein subset: {len(self.protein_subset)} proteins")
                logger.debug(f"  Timeout per protein: {self.protein_timeout_minutes} minutes")
        ...
        for idx, row in df.iterrows():
            ...
            if self.verbose:
                ...
                logger.info(f"Processing {structure_name} ({idx+1}/{stats['total']}) - sequence length: {seq_length}")
            else:
    ...
    def _predict_with_timeout(self, sequence: str, output_path: str, structure_name: str) -> bool:
        """
        ...
        try:
            if self.verbose:
                logger.debug(f"Starting ESMFold prediction for {structure_name} with {timeout_seconds}s timeout...")
            ...
            signal.alarm(0)  # Cancel timeout
            logger.warning(f"Prediction timed out for {structure_name} after {self.protein_timeout_minutes} minutes")
            raise
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Predict protein structures using ESMFold")
    ...
    parser.add_argument("reference_dir", help="Directory with reference ESMFold predictions")
