...
def compute_dssp_with_missing_handling(pdb_file, reference_sequence, model_index=0, chain_id=None):
    """
    ...
    if len(dssp_seq_from_pdb) == len(reference_sequence):
        # Direct mapping
        for i, (ref_aa, pdb_residue) in enumerate(zip(reference_sequence, protein_residues)):
        ...
        while ref_pos < len(reference_sequence):
            if pdb_pos < len(protein_residues):
                ...
                if dssp_key in dssp:
                    ...
                    if pdb_aa == reference_sequence[ref_pos] or reference_sequence[ref_pos] == 'X':
...
def compute_dssp(pdb_file, model_index=0, chain_id=None):
    ...
    protein_residues = [r for r in all_residues if r.get_id()[0] == ' ']  # Only standard amino acids
    ...
    for res_num in range(min_res, max_res + 1):
        if res_num in existing_residues:
            ...
            print(f"added X at res_num {res_num} bc res_num is not in existing residues")
            dssp_array.append('X')
            seq_dssp.append('X')
...
def process_single_entry(args):
    ...
    try:
        ...
        dssp, seq_dssp = compute_dssp_with_missing_handling(path, sample['seq'], chain_id=chain_id)
...
def main():
    ...
    for entry, result in tqdm(results, desc="Updating data"):
        if result['status'] == 'missing_pdb':
            ...
            missing_pdbs.append((
                entry, result['pdb_id'], result['chain_id'],
                result['seq_len'], result['dssp_len']
            ))
            
        elif result['status'] == 'misaligned':
            misaligned.append((
                entry, result['pdb_id'], result['chain_id'], 
                result['seq_len'], result['seq_dssp_len'], result['dssp_len']
            ))
            data_pdb[entry]['dssp'] = result['dssp']
            ...
            errors.append((
                entry, result['pdb_id'], result['chain_id'], 
                result['error'], result.get('traceback', ''),
                result['seq_len'], result['dssp_len']
            ))
    ...
    print(f"🎯 Total with DSSP data: {len(aligned) + len(misaligned) + len(missing_pdbs) + len(errors)}")
    ...
    with open('../datasets/cath-4.2/chain_set_map_with_b_factors_dssp.pkl', 'wb') as file:
    ...
    with open('missing_pdbs.txt', 'w') as file:
        for entry, pdb_id, chain_id, seq_len, dssp_len in missing_pdbs:
            file.write(f"{entry}: {pdb_id}.{chain_id}, len(seq): {seq_len}, len(dssp): {dssp_len} (X values)\n")
    ...
    with open('misaligned.txt', 'w') as file:
        for entry, pdb_id, chain_id, seq_len, seq_dssp_len, dssp_len in misaligned:
            file.write(f"{entry}: {pdb_id}.{chain_id}, len(seq): {seq_len}, len(seq_dssp): {seq_dssp_len}, len(dssp): {dssp_len}\n")
    ...
    if errors:
        with open('processing_errors.txt', 'w') as file:
            for entry, pdb_id, chain_id, error, traceback_str, seq_len, dssp_len in errors:
                file.write(f"Entry: {entry}\n")
                file.write(f"PDB: {pdb_id}.{chain_id}\n")
                file.write(f"Error: {error}\n")
                file.write(f"Sequence length: {seq_len}, DSSP length (X values): {dssp_len}\n")
